...
public class OrderDAO {
    ...
    public List<Order> getRecentOrdersByUser(int userId, int limit) {
        ...
        try {
            ...
            String sql = "SELECT * FROM \"Order\" WHERE user_id = ? ORDER BY order_date DESC FETCH FIRST ? ROWS ONLY";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, userId);
    ...
    public Map<String, Double> getSalesStatistics() {
        ...
        try {
            ...
            // Total sales
            String totalSql = "SELECT SUM(final_amount) AS total FROM \"Order\"";
            ResultSet rs = stmt.executeQuery(totalSql);
            if (rs.next()) {
                stats.put("totalSales", rs.getDouble("total"));
            }

            // Monthly sales
            String monthlySql = "SELECT SUM(final_amount) AS monthly FROM \"Order\" " +
                              "WHERE order_date >= CURRENT_DATE - 30 DAYS";
            rs = stmt.executeQuery(monthlySql);
            if (rs.next()) {
                stats.put("monthlySales", rs.getDouble("monthly"));
            }

            // Weekly sales
            String weeklySql = "SELECT SUM(final_amount) AS weekly FROM \"Order\" " +
                              "WHERE order_date >= CURRENT_DATE - 7 DAYS";
            rs = stmt.executeQuery(weeklySql);
            if (rs.next()) {
                stats.put("weeklySales", rs.getDouble("weekly"));
            }
